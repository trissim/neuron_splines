...
def get_edges(neuron):
  ...
  for edge in dgraph.edges:
    ...
    p1 = neuron.nodes.loc[neuron.nodes['node_id'] == edge[0]]
    p2 = neuron.nodes.loc[neuron.nodes['node_id'] == edge[1]]
...
import h5py
def ims_to_img(path_to_ims):
    ims_file = h5py.File(path_to_ims, 'r')
    img = np.array(ims_file['DataSet']['ResolutionLevel 0']['TimePoint 0']['Channel 0']['Data'])
    def get_last_2d(x):
        m,n = x.shape[-2:]
